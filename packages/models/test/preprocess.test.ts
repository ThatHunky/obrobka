...
describe('ваги моделей', () => {
  /**
   * Ваги показуються користувачу до завантаження, тож мають збігатися
   * з реальними файлами. Раніше сюди потрапило число з картки
   * HuggingFace у десяткових МБ, а показ іде в мебібайтах — вийшло
   * 11.8 замість 12.4.
   */
  it('вказані в байтах, а не в десяткових МБ', () => {
    const expected: Record<string, number> = {
      fast: 4_574_861,
      portrait: 12_984_781,
      quality: 88_152_708,
    };
    for (const m of MODELS) expect(m.bytes).toBe(expected[m.id]);
  });
});
