...
export function taskPages(): ToolEntry[] {
  return [
    ...biPage('compress-image', (locale) => locale === 'uk' ? {
      ...
      title: 'Стиснути зображення онлайн — без завантаження на сервер',
      description: 'Зменшіть вагу фотографії у браузері. Виміряно: WebP забирає 75 % '
        + 'від JPEG тієї ж якості, AVIF — 40 %.',
      h1: 'Стиснути зображення',
    ...
    ...biPage('upscale-image', (locale) => locale === 'uk' ? {
      ...
      title: 'Збільшити зображення нейромережею — удвічі або вчетверо, у браузері',
      description: 'Swin2SR домальовує деталі замість простого розтягування. Працює тайлами, '
        + 'тож пам’ять не залежить від розміру знімка.',
      h1: 'Збільшити зображення',
