...
<style>
  ...
  /*
   * Підказка під контролом.
   *
   * Від'ємне поле писалось під сітку віджета: там проміжок 1.5rem, і для
   * підпису до контрола над ним це забагато. Але воно діяло й усередині
   * fieldset, де проміжку немає взагалі — підказка під «Збільшенням»
   * лягала на чипи на 6 px. Не лише неохайно: у розмітці вона пізніше,
   * тож перехоплювала й натискання. Мишею це ховалось за наведенням,
   * яке піднімає чип, на дотику наведення немає — і нижній край кнопки
   * просто не працював. Тож підтягуємо лише там, де є що підтягувати.
   */
  .tip {
    font-size: 0.8rem;
    color: var(--fg-faint);
